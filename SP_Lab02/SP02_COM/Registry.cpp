	...
	const WCHAR* szProgID)			//   IDs
{
	...
	WCHAR szModule[512];
	DWORD dwResult = GetModuleFileName(hModule, szModule, sizeof(szModule) / sizeof(WCHAR));	// Получение путь к исполняемому файлу COM-сервера
	assert(dwResult != 0);

	WCHAR szCLSID[CLSID_STRING_SIZE];
	CLSIDtochar(clsid, szCLSID, sizeof(szCLSID));	// Преобразование clsid в строку

	WCHAR szKey[264];      //smw szKey[64]
	wcscpy_s(szKey, L"CLSID\\");
	wcscat_s(szKey, szCLSID);

	// Создание записи в реестре для класса COM-сервера и связанных идентификаторов
...

{
	...
	//  удаление записи в реестре для класса COM-сервера и связанных идентификаторов
