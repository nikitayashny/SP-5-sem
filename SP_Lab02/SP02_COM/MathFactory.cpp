...
STDMETHODIMP MathFactory::CreateInstance(LPUNKNOWN pUnkOuter, REFIID riid, void** ppvObj)   // создает новый объект класса Math
{
    HRESULT hr = E_UNEXPECTED;  // указатель на интерфейс, запрошенный через параметр riid
...
STDMETHODIMP  MathFactory::LockServer(BOOL fLock) { //  управляет блокировкой сервера
    if (fLock)
